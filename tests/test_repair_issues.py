...
# ── Part H: metadata backoff arithmetic ───────────────────────────────


class _FakeNameApi:
    """Return only the aliases a real Shelly account would carry.

    Mirrors ``get_device_names``: devices with no cloud alias (never
    renamed in the Shelly app) are omitted SILENTLY — no exception.
    """

    def __init__(self, names: dict[str, str]) -> None:
        self.names = names

    async def get_device_names(self, ids):
        return {d: n for d, n in self.names.items() if d in ids}


def _make_name_lookup_coordinator(monkeypatch, names: dict[str, str]):
    """A coordinator wired up just enough to run ``_refresh_device_names``."""
    # Skip the real 1.2 s rate-limit gap, and the device-registry push that
    # needs a live HA instance.
    monkeypatch.setattr(coordinator_mod, "_V2_NAME_LOOKUP_GAP_S", 0)
    monkeypatch.setattr(
        coordinator_mod, "dr", type("_dr", (), {
            "async_get": staticmethod(lambda _hass: _FakeDeviceRegistry()),
        }),
    ...
    coord.hass = None
    coord._entry = None
    coord._api = _FakeNameApi(names)
    coord.device_names = {}
    coord.devices = {}
    coord._name_lookup_in_flight = True
    coord._name_lookup_backoff = {}
    # The registry push and the listener nudge both need a live HA instance
    # and are irrelevant to backoff bookkeeping.
...
def test_metadata_backoff_delay(failures: int, expected: float) -> None:
    """Doubling, capped at one hour — never a permanent give-up."""
    assert metadata_backoff_delay(failures) == expected


def test_backoff_skips_then_becomes_eligible() -> None:
    backoff: dict[str, tuple[int, float]] = {}
    assert _backoff_elapsed(backoff, "dev0", 0.0) is True

    delay = _backoff_record_failure(backoff, "dev0", 100.0)
    assert delay == 60.0
    assert _backoff_elapsed(backoff, "dev0", 159.0) is False
    assert _backoff_elapsed(backoff, "dev0", 160.0) is True


def test_backoff_success_removes_entry() -> None:
    backoff: dict[str, tuple[int, float]] = {}
    _backoff_record_failure(backoff, "dev0", 0.0)
    _backoff_record_failure(backoff, "dev0", 60.0)
    assert backoff["dev0"][0] == 2
    backoff.pop("dev0", None)          # what a successful lookup does
    assert _backoff_elapsed(backoff, "dev0", 0.0) is True


def test_backoff_success_removes_entry_through_the_real_method(
    monkeypatch,
) -> None:
    """A resolved alias must actually pop the id, via production code.

    Popping it in the test would assert nothing about the coordinator.
    """
    coord = _make_name_lookup_coordinator(monkeypatch, {"dev0": "Kitchen"})
    _backoff_record_failure(coord._name_lookup_backoff, "dev0", 0.0)
    asyncio.run(coord._refresh_device_names(["dev0"]))
    assert "dev0" not in coord._name_lookup_backoff
    assert coord.device_names["dev0"] == "Kitchen"


def test_aliasless_device_is_backed_off_not_re_probed_every_poll(
    monkeypatch,
) -> None:
    """A device never renamed in the Shelly app must not cost a request/poll.

    ``get_device_names`` OMITS devices that carry no cloud alias and returns
    NORMALLY, so no exception is ever raised and the id never enters
    ``device_names``. Without recording that non-answer the id stays in the
    ``unresolved`` comprehension forever — one extra v1 request every poll,
    against the same 1 req/s budget the rate-limit repair reports. That
    would make the card blame the user for the integration's own traffic.
    """
    coord = _make_name_lookup_coordinator(monkeypatch, {})   # nothing has an alias
    asyncio.run(coord._refresh_device_names(["dev0", "dev1"]))
    assert set(coord._name_lookup_backoff) == {"dev0", "dev1"}
    assert coord._name_lookup_backoff["dev0"][0] == 1

    # The counter ESCALATES rather than resetting on each empty answer, so
    # the retry interval grows 60 s → 120 s → … instead of pinning at 60 s.
    asyncio.run(coord._refresh_device_names(["dev0"]))
    assert coord._name_lookup_backoff["dev0"][0] == 2

    # Bounded, never permanent: a rename in the app is still picked up.
    coord._api.names = {"dev0": "Kitchen"}
    coord._name_lookup_backoff["dev0"] = (2, 0.0)   # backoff expired
    asyncio.run(coord._refresh_device_names(["dev0"]))
    assert "dev0" not in coord._name_lookup_backoff


def test_partial_answer_backs_off_only_the_unanswered_ids(monkeypatch) -> None:
    coord = _make_name_lookup_coordinator(monkeypatch, {"dev0": "Kitchen"})
    asyncio.run(coord._refresh_device_names(["dev0", "dev1"]))
    assert "dev0" not in coord._name_lookup_backoff
    assert coord._name_lookup_backoff["dev1"][0] == 1
